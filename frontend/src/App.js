...
export default function App() {
  ...
  const DEFAULT_NOTIFS = [
    { id: 1, icon: "✅", title: "Booking Confirmed", msg: "Your slot was booked successfully.", ts: Date.now() - 2 * 60 * 1000, read: false },
    { id: 2, icon: "⏰", title: "Reminder", msg: "Your parking session has been active for 1 hour.", ts: Date.now() - 60 * 60 * 1000, read: false },
    { id: 3, icon: "🏷️", title: "Offer Available", msg: "Get 20% off on your next 3 bookings!", ts: Date.now() - 3 * 60 * 60 * 1000, read: true },
    { id: 4, icon: "🔒", title: "Security Alert", msg: "New login detected from your account.", ts: Date.now() - 24 * 60 * 60 * 1000, read: true },
  ];

  const [notifications, setNotifications] = useState([]);

  // Load notifications from localStorage once username is known
  useEffect(() => {
    if (!username) return;
    try {
      const saved = localStorage.getItem(`parksmart_notifs_${username}`);
      if (saved) {
        setNotifications(JSON.parse(saved));
      } else {
        // First time this user — set defaults with fresh timestamps
        const fresh = DEFAULT_NOTIFS.map(n => ({ ...n, ts: Date.now() - (DEFAULT_NOTIFS.indexOf(n) === 0 ? 2*60*1000 : DEFAULT_NOTIFS.indexOf(n) === 1 ? 60*60*1000 : DEFAULT_NOTIFS.indexOf(n) === 2 ? 3*60*60*1000 : 24*60*60*1000) }));
        setNotifications(fresh);
        localStorage.setItem(`parksmart_notifs_${username}`, JSON.stringify(fresh));
      }
    } catch {
  ...
  useEffect(() => {
    if (!username) return;
    try { localStorage.setItem(`parksmart_notifs_${username}`, JSON.stringify(notifications)); } catch {}
  }, [notifications, username]);

  // Helper: format real timestamp to "Xm ago" / "Xh ago" / "Xd ago"
  const timeAgoFromTs = (ts) => {
            ...
            {notifications.map(n => (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 5, opacity: 0.65 }}>🕐 {timeAgoFromTs(n.ts)}</div>
                </div>
              </div>
            ))}
                  ...
                  {notifications.map(n => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 4, opacity: 0.7 }}>{timeAgoFromTs(n.ts)}</div>
                      </div>
                    </div>
                  ))}
          ...
          <div className="page">
            ...
            <div className="map-page-layout">
              ...
              <div className="map-page-side">
                ...
                {/* Your Lot Status */}
                <div className="section" style={{ marginBottom: 16 }}>
                  <div className="section-title" style={{ marginBottom: 12 }}>🅿️ Your Lot Status</div>
                ...
                <div className="section">
                  <div className="section-title" style={{ marginBottom: 14 }}>📍 All Parking Lots</div>
                  {[
                    { id: 1, icon: "🅿️", name: "Connaught Place Parking", area: "Block F, CP", available: stats.free_slots ?? 20, total: stats.total_slots ?? 20, rate: 20, tag: "YOUR LOT", tagColor: "#00d4ff", lat: 28.6139, lng: 77.2090 },
                    { id: 2, icon: "🏢", name: "Palika Bazar Basement",   area: "Connaught Circus",      available: 34, total: 80, rate: 30, tag: "OPEN", tagColor: "#00e676", lat: 28.6268, lng: 77.2123 },
                    { id: 3, icon: "🛣️", name: "Hudson Lane Parking",     area: "GTB Nagar",             available: 12, total: 35, rate: 20, tag: "OPEN", tagColor: "#00e676", lat: 28.7041, lng: 77.1897 },
                    { id: 4, icon: "🏬", name: "Janpath Multi-Level",      area: "Janpath Road",          available: 0,  total: 40, rate: 40, tag: "FULL", tagColor: "#ff3d5a", lat: 28.6252, lng: 77.2198 },
                    { id: 5, icon: "🎭", name: "Mandi House Parking",      area: "Copernicus Marg",       available: 8,  total: 30, rate: 20, tag: "OPEN", tagColor: "#00e676", lat: 28.6185, lng: 77.2310 },
                  ].map(lot => (
                    <div key={lot.id} style={{
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 700, fontSize: 13, color: "var(--text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{lot.name}</div>
                          <div style={{ fontSize: 11, color: "var(--muted)" }}>{lot.area} · ₹{lot.rate}/hr</div>
                        </div>
                        <span style={{
                          background: lot.tagColor + "22", color: lot.tagColor,
                          border: `1px solid ${lot.tagColor}44`,
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                        ...
                        <span style={{ fontSize: 11, color: lot.available === 0 ? "#ff3d5a" : "#00e676", fontWeight: 600, flexShrink: 0 }}>
                          {lot.available === 0 ? "Full" : `${lot.available} free`}
                        </span>
                      </div>
