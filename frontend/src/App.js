...
function LiveTimer({ bookedAt }) {
  ...
  useEffect(() => {
    const tick = () => setElapsed(Math.floor((Date.now() - new Date(bookedAt)) / 1000));
    tick();
                ...
                {activeBooking ? (
                  <div className="section active-session-card">
                    ...
                    <div className="active-session-timer-row">
                      ...
                      <div><div className="active-session-timer-label">Est. Cost</div><span className="active-session-cost">₹{Math.max(10, Math.ceil((Date.now() - new Date(activeBooking.booked_at)) / 1800000) * 10)}</span></div>
                    </div>
                    ...
                    {history.slice(0, 3).map(b => (
                      <div key={b.id} className="recent-booking-row">
                        ...
                        <div className="recent-booking-fare">
                          {b.status === "completed" && b.released_at ? `₹${Math.max(10, Math.ceil((new Date(b.released_at) - new Date(b.booked_at)) / 1800000) * 10)}` : <span className={`status-badge status-${b.status}`}>{b.status}</span>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="active-page-layout">
                <div className="section active-page-card">
                  ...
                  <div className="active-page-meta">
                    ...
                    <div className="active-page-meta-item"><div className="active-page-meta-label">Est. Cost</div><div className="active-page-cost">₹{Math.max(10, Math.ceil((Date.now() - new Date(activeBooking.booked_at)) / 1800000) * 10)}</div></div>
                    ...
                    <tbody>
                      {history.map((b, i) => (
                        <tr key={b.id}>
                          ...
                          <td style={{ color: "var(--green)", fontWeight: 600 }}>{b.status === "completed" && b.released_at && b.booked_at ? "₹" + Math.max(10, Math.ceil((new Date(b.released_at) - new Date(b.booked_at)) / 1800000) * 10) : "—"}</td>
