...
export default function App() {
  ...
  const PARKING_DB = [
    ...
    { id: 3, name: "Shivaji Stadium Parking", area: "Baba Kharak Singh Marg", available: 12, total: 50, rate: 20, distance: "0.7 km", icon: "🏟️", tag: "OPEN", tagColor: "#00e676" },
  ...
  const ChangePasswordModal = () => {
    ...
    const handleSubmit = () => {
      ...
      setTimeout(() => { setShowChangePassword(false); setSuccess(false); }, 1500);
    };
    return (
      <div className="nlm-overlay" onClick={() => setShowChangePassword(false)}>
        <div className="nlm-modal" style={{ maxWidth: 380 }} onClick={e => e.stopPropagation()}>
          <div className="nlm-header">
            ...
            <button className="nlm-close" onClick={() => setShowChangePassword(false)}>✕</button>
          </div>
          {success ? (
            ...
            <>
              {["Current Password", "New Password", "Confirm New Password"].map((label, i) => (
                <div key={i} style={{ marginBottom: 12 }}>
                  ...
                  <input
                    type="password" className="nlm-input"
                    placeholder={label}
              ...
              {err && <div className="nlm-err">{err}</div>}
              <button className="nl-book-btn" style={{ marginTop: 16 }} onClick={handleSubmit}>Update Password →</button>
            </>
          )}
        </div>
      </div>
            ...
            <div style={{ position: "relative" }}>
              ...
              {showHelpPanel && (
                <div style={{
                  ...
                  {[
                    { icon: "🅿️", title: "How to Book a Slot", desc: "Go to Book a Slot → select a slot → enter vehicle number → confirm." },
                    { icon: "🤖", title: "AI Slot Recommendation", desc: "Use the 'Find Best Slot with AI' button for smart suggestions." },
                    { icon: "📱", title: "QR Code Entry", desc: "After booking, show your QR code at the parking gate." },
                    { icon: "💳", title: "Pricing", desc: "₹20/hr at ParkSmart CP. Rates vary at nearby lots." },
                    { icon: "🔓", title: "Release a Slot", desc: "Go to Active Booking and click 'End Session' when done." },
                  ].map((item, i) => (
                    <div key={i} style={{
                      display: "flex", gap: 12, padding: "12px 16px",
                  ...
                  <div style={{ padding: "10px 16px", textAlign: "center" }}>
                    <a href="mailto:support@parksmart.in" style={{ color: "var(--accent)", fontSize: 12, fontWeight: 600, textDecoration: "none" }}>📧 Contact Support</a>
                  </div>
                </div>
              )}
            </div>
