...
function MapView({ slots, stats, activeBooking }) {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById("leaflet-js")) {
      ...
      // Script tag already exists but may still be loading — poll
  ...
  // Step 2: Init map and draw markers once Leaflet is ready
  useEffect(() => {
    ...
    Object.entries(zoneSlots).forEach(([zone, zSlots]) => {
      ...
      zSlots.forEach((s, i) => {
        ...
        const slotIcon = L.divIcon({
          className: "",
          html: `<div style="width:36px;height:36px;background:${color};color:#000;font-weight:800;font-size:11px;border-radius:8px;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.3);border:2px solid rgba(255,255,255,0.3);">${label}</div>`,
          iconSize: [36, 36],
          iconAnchor: [18, 18],
        });
