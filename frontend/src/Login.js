...
function Login({ setLoggedIn, setUsername, setIsAdmin }) {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("parksmart_remember");
      if (saved) {
  ...
  const handleGoogleResponse = async (response) => {
    try {
      ...
      await handleGoogleUserInfo({ email: payload.email, name: payload.name, sub: payload.sub });
    } catch { setError("Google sign-in failed. Please try again."); }
  };

  const handleGoogleClick = () => {
    const CLIENT_ID = process.env.REACT_APP_GOOGLE_CLIENT_ID || "";
    if (!CLIENT_ID) { setModal("googleSetup"); return; }

    if (window.google?.accounts?.id) {
      // Initialize fresh each click — forces account chooser every time
      ...
      // Use oauth2 token client for popup — always shows account chooser
      const tokenClient = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (tokenResponse) => {
          ...
          try {
            ...
            await handleGoogleUserInfo(info);
          } catch { setError("Google sign-in failed. Please try again."); }
        },
      });
      ...
      setModal("googleSetup");
    }
  };

  // Shared Google auth handler — called after getting user info
  const handleGoogleUserInfo = async (info) => {
    try {
      setLoading(true);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res.ok && data.requires_otp) {
        ...
        if (rememberMe) {
          localStorage.setItem("parksmart_remember", JSON.stringify({ username, password }));
        } else {
          localStorage.removeItem("parksmart_remember");
        }
  ...
  return (
    <div className="lp-root">
      ...
      {modal === "googleSetup" && (
        <Modal title="🔑 Google Sign In Setup" onClose={() => setModal(null)}>
          <div style={{ fontSize: 13, color: "#6b7280", lineHeight: 1.7 }}>
            <p style={{ marginBottom: 12 }}>Google Sign In needs a <strong style={{color:"#1a1d2e"}}>Client ID</strong> to work. Follow these steps:</p>
            <div style={{ background: "#f7f8fc", borderRadius: 10, padding: "14px 16px", marginBottom: 14 }}>
              <div style={{ fontWeight: 700, color: "#1a1d2e", fontSize: 12, marginBottom: 8 }}>STEPS TO ENABLE:</div>
              {[
                "Go to console.cloud.google.com",
                "Select your ParkSmart project",
                "APIs & Services → Credentials",
                "Copy your OAuth 2.0 Client ID",
                "Vercel → Settings → Environment Variables",
                "Key: REACT_APP_GOOGLE_CLIENT_ID → paste Client ID",
                "Redeploy on Vercel"
              ].map((s, i) => (
                <div key={i} style={{ display: "flex", gap: 8, marginBottom: 5 }}>
                  <span style={{ background: "#4361ee", color: "#fff", borderRadius: "50%", width: 18, height: 18, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, flexShrink: 0, marginTop: 2 }}>{i+1}</span>
                  <span>{s}</span>
                </div>
              ))}
            </div>
            <p>Also add this in Google Console → <strong style={{color:"#1a1d2e"}}>Authorized JavaScript origins:</strong><br/>
              <code style={{background:"#f0f2fb", padding:"3px 8px", borderRadius:4, fontSize:11, display:"inline-block", marginTop:4}}>https://smart-parking-system-using-ai-ml.vercel.app</code>
            </p>
          </div>
          <button className="lp-btn-primary" style={{ marginTop: 16 }}
            onClick={() => { setModal(null); window.open("https://console.cloud.google.com/apis/credentials", "_blank"); }}>
            Open Google Console →
          </button>
        </Modal>
      )}
          ...
          {otpStep ? (
            <div className="lp-form-wrap">
              <div style={{ textAlign: "center", marginBottom: 8 }}>
                ...
                <h2 className="lp-welcome" style={{ textAlign: "center" }}>Verify It's You</h2>
                <p className="lp-sub" style={{ textAlign: "center" }}>
                  We sent a 6-digit code to <strong style={{ color: "#1a1d2e" }}>{otpEmailHint}</strong>
                </p>
              </div>

              {/* OTP boxes */}
              <div style={{ display: "flex", gap: 10, justifyContent: "center", margin: "24px 0 8px" }} onPaste={handleOtpPaste}>
                {otpCode.map((digit, idx) => (
                  <input
                    ...
                    style={{
                      ...
                      border: `2px solid ${digit ? "#4361ee" : "#e4e7f0"}`,
                      borderRadius: 12, background: digit ? "#eef0fd" : "#f7f8fc",
                      color: "#1a1d2e", outline: "none", fontFamily: "monospace",
              ...
              <div style={{ textAlign: "center", marginBottom: 16 }}>
                {otpTimer > 0 ? (
                  <span style={{ fontSize: 12, color: "#8a91b4" }}>
                    Code expires in <strong style={{ color: otpTimer < 60 ? "#e63946" : "#4361ee" }}>
                      {String(Math.floor(otpTimer / 60)).padStart(2, "0")}:{String(otpTimer % 60).padStart(2, "0")}
                    </strong>
                  </span>
                ) : (
                  <span style={{ fontSize: 12, color: "#e63946", fontWeight: 600 }}>OTP expired!</span>
                )}
              </div>

              {otpError && (
                <div className={otpError.includes("sent") ? "lp-success" : "lp-error"} style={{ marginBottom: 12 }}>
                  {otpError}
                </div>
              )}

              <button className="lp-btn-primary" onClick={handleVerifyOtp} disabled={otpLoading || otpCode.join("").length < 6}>
                {otpLoading ? "Verifying..." : "Verify & Sign In →"}
              </button>

              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16, alignItems: "center" }}>
                <button className="lp-back-btn" style={{ marginBottom: 0 }}
                  onClick={() => { setOtpStep(false); setOtpCode(["","","","","",""]); setOtpError(""); }}>
                  ← Back
                </button>
                <button className="lp-forgot" onClick={handleResendOtp} disabled={otpTimer > 240}>
                  Resend Code
                </button>
              </div>
            </div>
