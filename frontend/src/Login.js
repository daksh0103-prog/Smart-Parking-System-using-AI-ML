...
function Login({ setLoggedIn, setUsername, setIsAdmin }) {
  ...
  const handleGoogleResponse = async (response) => {
    try {
      ...
      if (loginRes.ok) { setUsername(googleUsername); setLoggedIn(true); return; }
      ...
      if (regRes.ok) { setUsername(googleUsername); setLoggedIn(true); }
      else { const d = await regRes.json(); setError(d.detail || "Google sign-in failed."); }
      ...
      const tokenClient = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (tokenResponse) => {
          ...
          try {
            ...
            const googleUsername = (info.name || info.email.split("@")[0])
              .replace(/\s+/g, "_").toLowerCase();

            // Try login first
            const loginRes = await fetch(`${API}/login`, {
              method: "POST", headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ username: googleUsername, password: `google_${info.sub}` }),
            });
            if (loginRes.ok) { setUsername(googleUsername); setLoggedIn(true); return; }
            ...
            const regRes = await fetch(`${API}/register`, {
              method: "POST", headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ username: googleUsername, email: info.email, password: `google_${info.sub}` }),
            });
            if (regRes.ok) { setUsername(googleUsername); setLoggedIn(true); }
            else { const d = await regRes.json(); setError(d.detail || "Google sign-in failed."); }
          } catch { setError("Google sign-in failed. Please try again."); }
